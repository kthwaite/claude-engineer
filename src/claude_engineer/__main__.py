...
from .voice import VoiceMode, test_voice_mode
...
def validate_ai_response(response_text):
    if isinstance(response_text, list):
        ...
        logging.debug(
            f"validate_ai_response received type {type(response_text)}: {response_text}"
        )
        raise ValueError(
...
async def edit_and_apply_multiple(files, project_context, is_automode: bool = False):
    global file_contents
    results = []
    console_outputs = []
        ...
        try:
            ...
            if edit_instructions:
                ...
                (
                    ...
                    failed_edits,
                    console_output,
                ) = await apply_edits(path, edit_instructions, original_content)
            ...
            results.append({"path": path, "status": "error", "message": error_message})
...
async def send_to_ai_for_executing(code, execution_result):
    ...
    try:
        ...
        code_execution_tokens["cache_creation"] = (
...
async def execute_tool(tool_name: str, tool_input: Dict[str, Any]) -> Dict[str, Any]:
    try:
        ...
        if tool_name == "create_files":
            ...
            if not is_error:
                result, console_output = await edit_and_apply_multiple(
                    files, tool_input["project_context"], is_automode=automode
                )
            ...
            if tool_name == "create_files":
                ...
                for result in edit_results:
                    if isinstance(result, dict) and result.get("status") in [
                        ...
                        file_contents[result["path"]] = result.get(
                            "edited_content", file_contents.get(result["path"], "")
                        )
            elif tool_name == "read_multiple_files":
    ...
    while True:
        if voice_mode:
            ...
            if user_input is None:
                ...
                console.print(
                    Panel(
                        "Exited voice input mode due to error. Returning to text input.",
                        style="bold yellow",
                    )
                )
                continue
            ...
            if not stay_in_voice_mode:
                ...
                console.print(
                    Panel(
                        "Exited voice input mode. Returning to text input.",
                        style="bold green",
                    )
                )
                if command_result:
        ...
        if user_input.lower() == "test voice":
            await test_voice_mode(console, save_chat, reset_conversation)
            continue
        ...
        if user_input.lower() == "voice":
            ...
            console.print(
                Panel(
                    "Entering voice input mode. Say 'exit voice mode' to return to text input.",
                    style="bold green",
                )
            )
            continue
